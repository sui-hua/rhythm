 ...
 * 日程表构建工具
 *
 * 将任务、goal_day、习惯三类数据源合并为统一的日程列表。
 * 纯函数，无副作用，供 dayStore 和 useDayExecutionItems 复用。
 */
...
/**
 * 格式化顺延来源日期标签
 * @param dateOnly - 日期字符串 YYYY-MM-DD
 */
const formatCarryOverLabel = (dateOnly: string): string => {
...
export function buildDayExecutionItems({
    ...
    tasks.forEach(task => {
        ...
        schedule.push({
            ...
            scheduledDate: targetDateStr || toDateOnly(startDate),
    ...
    goalDays.forEach(plan => {
        let startHourValue: number | undefined
        ...
        if (inheritedTime) {
            const parts = inheritedTime.split(':')
        ...
        schedule.push({
            ...
            durationHours: durationInHours,
            rawDuration: durationInHours,
            scheduledDate: targetDateStr || plan.day,
            time: startTimeString,
    ...
    habits.forEach(habit => {
        ...
        if (habit.task_time) {
            const parts = habit.task_time.split(':')
            const hours = Number(parts[0]) || 0
            const minutes = Number(parts[1]) || 0
            startHourValue = hours + minutes / 60
            startTimeString = `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}`
        ...
        schedule.push({
            ...
            durationHours: durationInHours,
            rawDuration: durationInHours,
            scheduledDate: targetDateStr || undefined,
            time: startTimeString,
            duration: durationString,
            category: '日常习惯',
            title: habit.title || '',
