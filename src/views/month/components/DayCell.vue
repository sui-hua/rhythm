<template>
  <div
      ...
      class="bg-white p-2 sm:p-4 flex flex-col transition-all relative overflow-hidden flex-1 min-h-0"
      :class="[
        day.isCurrent 
          ? 'cursor-pointer group z-10' 
          : 'bg-transparent cursor-default opacity-10'
      ]"
  >
    <!-- Hover Bottom Border -->
    <div v-if="day.isCurrent" class="absolute bottom-0 left-0 right-0 h-0 group-hover:h-1 bg-black transition-all duration-300 ease-out z-20"></div>
      ...
      <div class="flex flex-col gap-1">
        ...
        <div v-if="day.isCurrent && day.tasks?.length" class="flex gap-0.5 h-1">
          <div 
            ...
            :key="i"
            class="w-1 h-1 bg-black/20 group-hover:bg-black transition-colors duration-300"
          ></div>
          <div v-if="day.tasks.length > 5" class="w-1 h-1 border-b border-r border-black/20 group-hover:border-black rotate-45 transform -translate-y-0.5 ml-0.5"></div>
        </div>
        
        <div class="flex items-baseline gap-1">
          <span 
            class="text-4xl sm:text-5xl lg:text-6xl font-black tracking-tighter leading-none transition-colors duration-500"
            :class="[day.isCurrent ? 'text-black' : 'text-zinc-300']"
          >
            {{ day.date }}
          </span>
        </div>
    ...
    <div v-if="day.isCurrent" class="absolute right-0 top-0 bottom-0 w-6 sm:w-8 flex flex-col justify-between py-4 sm:py-6 z-10">
      <div v-for="h in 24" :key="h" 
           ...
           :class="[
             day.taskHours?.includes(h) 
               ? 'w-4 sm:w-6 bg-black group-hover:w-5 sm:group-hover:w-7 group-hover:bg-black' 
