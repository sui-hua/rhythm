 ...
 * @description 通用总结表单 Composable，提供总结的标题、内容、心情状态的响应式管理。
 *              支持日/周/月/年四种总结类型，自动解析不同数据格式并生成对应的表单占位文本。
 ...
 * 通用总结表单 Composable
 * 
 * @description 管理总结表单的状态（标题、内容、心情），响应 initialDataRef 的变化自动填充表单，
 ...
 * @param {import('vue').Ref<string>} typeRef - 总结类型引用，支持 'day' | 'week' | 'month' | 'year'
 ...
 * @returns {import('vue').Ref<string|null>} returns.mood - 心情状态
 * @returns {import('vue').ComputedRef<string>} returns.typeName - 总结类型的中文名称（日/周/月/年）
 * @returns {import('vue').ComputedRef<string>} returns.placeholderText - 表单占位提示文本
 * @returns {function(): {title: string, text: string, mood: string|null}} returns.buildPayload - 构建提交数据结构
 * 
 * @example
 * const initialDataRef = ref({ title: '本周总结', content: { text: '做了很多事' }, mood: 'happy' })
 * const typeRef = ref('week')
 * const { title, content, mood, typeName, placeholderText, buildPayload } = useGenericSummaryForm(initialDataRef, typeRef)
 */
export const useGenericSummaryForm = (initialDataRef, typeRef) => {
  ...
  const content = ref('')
  /** @type {import('vue').Ref<string|null>} 心情状态 */
  const mood = ref(null)
   ...
   * 
   * 数据兼容处理：
   * - content 为对象且非数组时，取其 text 和 mood 字段
  ...
  watch(
    ...
    (newVal) => {
      if (newVal) {
        // 解析 content 结构：支持 { text, mood } 格式或直接字符串
  ...
  /**
   * 构建提交用的数据载荷
   * @returns {{title: string, text: string, mood: string|null}} 整理后的数据结构
   */
  const buildPayload = () => ({
