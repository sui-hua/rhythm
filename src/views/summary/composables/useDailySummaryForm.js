 ...
 * 表单包含四个字段：done（今日完成）、improve（改进方向）、
 * tomorrow（明日计划）、mood（今日心情）。
 * 
 ...
 *   {
 *     content: { done: '', improve: '', tomorrow: '', mood: null },
 *     mood: null
 *   }
 * @returns {
 *   {
 *     formData: Ref<Object>, - 表单数据对象，包含 done/improve/tomorrow/mood
...
export const useDailySummaryForm = (initialDataRef) => {
   ...
   * - tomorrow: 明日计划
   * - mood: 今日心情（可选）
   * @type {Ref<{done: string, improve: string, tomorrow: string, mood: *}>}
   */
  const formData = ref({
    done: '',
    improve: '',
   ...
   * 解析数据并更新 formData。数据来源优先级：
   * 1. content.done / content.improve / content.tomorrow
   * 2. content.mood / mood（顶层 mood 字段）
   * 
