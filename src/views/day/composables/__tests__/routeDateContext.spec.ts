...
import type { RouteDateContext, RouteYearContext, RouteMonthContext } from '@/views/day/utils/types'

describe('routeDateContext', () => {
  it('从完整 year/month/day 路由解析目标日期', () => {
    const context: RouteDateContext = getRouteDateContext({
      year: '2027',
      month: '1',
      day: '1'
    })
  ...
  it('缺少路由参数时使用 fallbackDate', () => {
    const fallback: Date = new Date(2026, 3, 18)
    const context: RouteDateContext = getRouteDateContext({}, fallback)
  ...
  it('year 路由参数可解析但非 canonical 时保留解析值', () => {
    const context: RouteYearContext = getRouteYearContext('02026', 2025)
  ...
  it('0-99 的 year 路由参数不会被当成 canonical 年份', () => {
    const context: RouteYearContext = getRouteYearContext('1', 2025)
  ...
  it('year 路由参数完全无效时回退到 fallbackYear', () => {
    const context: RouteYearContext = getRouteYearContext('abc', 2025)
  ...
  it('month 路由参数可解析但非 canonical 时保留解析值', () => {
    const context: RouteMonthContext = getRouteMonthContext('02026', '04', new Date(2025, 0, 1))
  ...
  it('0-99 的 year 出现在 month 路由时不会被当成 canonical', () => {
    const context: RouteMonthContext = getRouteMonthContext('1', '4', new Date(2025, 0, 1))
  ...
  it('能正确生成日/月/年路由', () => {
    const date: Date = new Date(2027, 0, 1)
