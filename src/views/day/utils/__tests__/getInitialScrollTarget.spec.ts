...
import type { ScrollTarget } from '@/views/day/utils/types'

describe('getInitialScrollTarget', () => {
  const today: Date = new Date(2026, 3, 19)
  const now: Date = new Date(2026, 3, 19, 15, 30)

  it('无任务时回到默认 08:00', () => {
    const result: ScrollTarget = getInitialScrollTarget({
      schedule: [],
      targetDate: today,
      now
    })
